...
public class RankupInventory {
   ...
   static {
      LobbyHost.getInstance().getServerWatcherManager().watch((new ServerWatcher() {
         @Override
      ...
      LobbyHost.getInstance().getServerManager().getBalancer(ServerType.RANKUP).getList().forEach(server -> {
